...
function App() {
  ...
  const getComments = (id) =>
    fetch(`${api}/comments?postId=${id}`).then((response) => response.json());

  const getUserPosts = (id) =>
    fetch(`${api}/posts?userId=${id}`).then((response) => response.json());
        ...
        <Grid item xs={6}>
          ...
          <FormControl variant="outlined" className={classes.formControl}>
            ...
            >
              ...
              {state.users &&
                state.users.map((x, i) => (
                  <option value={state.users[i].id} key={state.users[i].id}>
                    {state.users[i].name}
                  </option>
                ))}
            </Select>
          ...
          [
            ...Array(
              state.count + state.additionalViews > state.posts.length
                ? state.posts.length
                : state.count + state.additionalViews
            ),
          ].map((e, i) => (
              ...
              <Card className={classes.root} variant="outlined">
                ...
                <CardActions>
                  ...
                  {state.posts[i].totalComments && (
                    <Badge
                      badgeContent={state.posts[i].totalComments}
                      color="primary"
                    >
                      <CommentIcon />
                    </Badge>
